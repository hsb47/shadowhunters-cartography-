# ShadowHunters Multi-Cloud Cartography - AWS Ingestion Runner
# Performs environment validation and executes read-only Cartography AWS sync.

[CmdletBinding()]
param(
    [string]$SelectedModules = "create-indexes,aws,analysis",
    [string]$OutputFile = "$PSScriptRoot\..\output\cartography-aws.log"
)

$ErrorActionPreference = "Stop"

Write-Host "============================================================" -ForegroundColor Cyan
Write-Host " STARTING AWS CLOUD INGESTION (READ-ONLY)" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan

# 1. Validate Environment & Credentials
& "$PSScriptRoot\check-environment.ps1" -RequireAws
if ($LASTEXITCODE -ne 0) {
    Write-Error "AWS environment validation failed. Aborting ingestion."
    exit 1
}
...
Write-Host "`nExecuting Cartography AWS Ingestion..." -ForegroundColor Yellow
...
if ($process.ExitCode -eq 0) {
    Write-Host "`n[SUCCESS] AWS Ingestion finished successfully." -ForegroundColor Green
    Write-Host "Log details saved to: $OutputFile" -ForegroundColor Gray
} else {
    Write-Host "`n[FAIL] AWS Ingestion process failed with exit code $($process.ExitCode)." -ForegroundColor Red
